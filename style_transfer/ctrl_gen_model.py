...
class CtrlGenModel(object):
    ...
    def _build_model(self, inputs, vocab, gamma, lambda_g, lambda_z, lambda_z1, lambda_z2, lambda_ae):
        ...
        if self._hparams.WGAN:
            ...
            normalized_d_logits = d_logits - tf.math.reduce_max(d_logits, axis = 0, keepdims=True)
            W = tf.cast(get_batch_size(inputs['text_ids']),tf.float32) * tf.exp(normalized_d_logits) / tf.reduce_sum(tf.exp(normalized_d_logits), 0)
        ...
        train_op_g = get_train_op(
            loss_g, g_vars, hparams=self._hparams.opt_g)
        ...
        train_op_c = get_train_op(
            loss_clas, clas_vars, hparams=self._hparams.opt)
